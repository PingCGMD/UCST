...
liste_name = ["gyrate.xvg","gyrate-a.xvg"]
...
#    output_file = open("{}_average_".format(str_temperature)+i, "w")
    ...
    for i in liste_name:
        ...
        for j in range(index2,27+len(lines1[index2:])):
            X.append(float(lines1[j][1:11]))
       #    print(X)
            Y1.append((float(lines1[j][15:23])+float(lines2[j][15:23]))/2)

...
liste_name = ["AAm_mindist.xvg", "AAm_numcount.xvg", "BMDO_numcount.xvg", "BMDO_mindist.xvg", "BMDO_AAm_mindist.xvg", "BMDO_AAm_numcount.xvg"]
...
#    output_file = open("{}_average_".format(str_temperature)+i, "w")
    ...
    for i in liste_name:
        ...
        for j in range(index2,24+len(lines1[index2:])):
            X.append(float(lines1[j][0:8]))
       #    print(X)
            Y1.append((float(lines1[j][18:22])+float(lines2[j][18:22]))/2)
